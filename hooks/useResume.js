...
export default function useResume(template) {
  const [resume, setResume] = useState({
    ...DEFAULT_RESUME,
    template: template || DEFAULT_RESUME.template,
  });
  ...
  useEffect(() => {
    ...
    const mergedResume = {
      ...
      template: template || stored?.template || DEFAULT_RESUME.template,
    };
  ...
  const toggleSection = (id) =>
    update((r) => ({
      ...r,
      sections: { ...r.sections, [id]: !r.sections[id] },
    }));

  /* ── Template / theme ── */
  const setTemplate = (tpl) =>
    update((r) => ({ ...r, template: tpl }));

  const setColorTheme = (c) =>
    update((r) => ({ ...r, colorTheme: c }));
  ...
  /* ── Skills ── */
  const addSkill = (s) =>
    update((r) => ({ ...r, skills: [...r.skills, s] }));

  const removeSkill = (i) =>
    update((r) => ({
      ...r,
      skills: r.skills.filter((_, idx) => idx !== i),
    }));
